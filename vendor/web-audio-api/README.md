# Vendored web-audio-api v1.2.0

Forked from [web-audio-api](https://crates.io/crates/web-audio-api) v1.2.0.

## Local changes

### Unbounded control channel (`src/io/mod.rs`)

Changed the control message channel from `crossbeam_channel::bounded(256)` to `crossbeam_channel::unbounded()`.

The bounded channel blocks the calling thread when more than 256 node operations (create, connect, disconnect, drop) are sent before the render thread drains them. This causes multi-second freezes during bulk operations that rapidly create/destroy many nodes (e.g. loading sample libraries with 80+ players). The render thread drains all pending messages every render quantum, so the queue doesn't grow unbounded in practice.

### ConvolverNode stereo fix (`src/node/convolver.rs`)

Removed `output.force_mono()` from `ConvolverRenderer::process()`. The original implementation unconditionally collapsed the convolver output to 1 channel before the processing logic ran. The match arms below it already correctly handle all multi-channel combinations (1→1, 1→2, 2→1, 2→2, 2→4, 1→4) and set the output channel count explicitly. The forced mono truncation made reverb (and anything downstream) sound mono regardless of the impulse response channel count.
