...
pub(crate) fn thread_init() -> (ControlThreadInit, RenderThreadInit) {
    ...
    // Unbounded so the control thread never blocks when many nodes are
    // created/destroyed rapidly. The render thread drains all pending messages
    // each quantum so the queue stays small in practice.
