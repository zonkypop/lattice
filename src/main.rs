// src/main.rs - Updated with XR mode support via --xr flag
...
async fn create_main_worker(script_path: &str) -> Result<(MainWorker, ModuleSpecifier, SharedArrayBufferStore), AnyError> {
    ...
    // Set __nativeXR flag before any modules run
...
fn run_xr_mode(script_path: &str) -> Result<(), AnyError> {
    ...
    rt.block_on(async {
        ...
        // Run event loop
        loop {
            // Check if XR session signaled exit
            if should_exit_xr() {
                info!("XR session ended, exiting immediately...");
                std::process::exit(0);
            }
            ...
            // Pump microtasks/promises
            let _ = worker.js_runtime.execute_script(
                "<xr_tick>",
                deno_core::ModuleCodeString::from_static(
                    "if (globalThis.__xrTick) { globalThis.__xrTick(); }"
                ),
            );
        }
